...
import HeaderSection from "../RoomChatPage/components/HeaderSection";
import type { PlaylistItem } from "../RoomChatPage/types";
...
const buildEditableItemsFromDb = (
...
const EditPage = () => {
  ...
  const displayUsername =
    localStorage.getItem(STORAGE_KEYS.username) ?? TEXT.notSet;
    ...
    const ensureAndLoad = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_URL}/collections?owner_id=${encodeURIComponent(
            clientId
          )}&pageSize=50`
        );
  ...
  }, [API_URL, clientId, displayUsername]);

  useEffect(() => {
    if (!API_URL || !activeCollectionId) return;
    let active = true;
    ...
    const loadItems = async () => {
      ...
      try {
        const res = await fetch(
          `${API_URL}/collections/${activeCollectionId}/items?pageSize=200`
        );
        const payload = await res.json().catch(() => null);
        if (!res.ok) {
  ...
  const handleFetchPlaylist = async () => {
    ...
    if (!playlistId) {
      setPlaylistError(TEXT.playlistErrorInvalid);
      return;
    }
    if (!PLAYLIST_API_URL) {
      setPlaylistError(TEXT.playlistErrorApi);
      return;
    }
    ...
    try {
      ...
      const payload = await res.json().catch(() => null);
      if (!res.ok) {
        throw new Error(payload?.error ?? TEXT.playlistErrorLoad);
      }
      const items = (payload?.items ?? []) as PlaylistItem[];
      if (items.length === 0) {
        throw new Error(TEXT.playlistErrorEmpty);
      }
      ...
      const nextItems = buildEditableItems(uniqueItems);
      setPlaylistItems((prev) => [...prev, ...nextItems]);
      if (wasEmpty && nextItems.length > 0) {
        setSelectedIndex(0);
      }
  ...
  const handleEndChange = (value: number) => {
    ...
    setCurrentTimeSec((prev) =>
      Math.min(Math.max(prev, nextStart), next)
    );
  ...
  const syncItemsToDb = async (collectionId: string) => {
    ...
    if (toUpdate.length > 0) {
      await Promise.all(
        toUpdate.map((item) =>
          fetch(`${API_URL}/collection-items/${item.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              sort: item.sort,
    ...
    if (pendingDeleteIds.length > 0) {
      await Promise.all(
        pendingDeleteIds.map((id) =>
          fetch(`${API_URL}/collection-items/${id}`, { method: "DELETE" })
        )
      );
      setPendingDeleteIds([]);
    }
        ...
        <div className="w-full md:w-full lg:w-3/5 mx-auto space-y-4">
          ...
          <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-4 space-y-4">
            <div className="grid gap-3">
              ...
              <label className="text-xs text-slate-300">{TEXT.collectionName}</label>
              <input
          ...
          {playlistItems.length > 0 && (
            <div className="space-y-3">
              <div className="text-sm text-slate-300">
                {TEXT.playlistCount}{playlistItems.length}{TEXT.songsUnit}
              </div>
              <div className="text-xs text-slate-400">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="px-2 py-1">
                        ...
                        <div className="text-[10px] text-slate-500">
                          {CLIP_DURATION_LABEL} {formatSeconds(Math.max(0, item.endSec - item.startSec))}
                        </div>
                      </div>
              ...
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3">
                ...
                <div className="text-xs text-slate-400">
                  {CLIP_DURATION_LABEL}: {formatSeconds(selectedClipDurationSec)}
                </div>
                <div className="mt-2">
              ...
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-4 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  />
                  <div className="flex items-center justify-between text-xs text-slate-400">
                    <span>{TEXT.start} {formatSeconds(startSec)}</span>
                    <span>{TEXT.end} {formatSeconds(endSec)}</span>
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  <label className="text-xs text-slate-300">{TEXT.answer}</label>
                  <input
                    value={answerText}
                    onChange={(e) => {
